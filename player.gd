...
func move(gravity: float, delta: float) -> void:
	...
	if stats.isWindBlow and not stats.resistWind:
		velocity.x = move_toward(velocity.x/10.0, movement * RUN_SPEED, acceleration * delta)
	else:	
...
func get_next_state(state: State) -> int:
	...
	match state:
		State.IDLE:
			...
			if should_slide() and stats.canDash:
		...
		State.FALL:
			...
			if can_wall_slide() and stats.slideAbility:
		...
		State.WALL_SLIDING:
			if (jump_request_timer.time_left > 0 ) and stats.climbAbility:
		...
		State.ATTACK_1:
			if not animation_player.is_playing():
				return State.ATTACK_2 if is_combo_requested and stats.canAttack3 else State.IDLE
	...
	match to:
		...
		State.HURT:
			...
			if (velocity.x*velocity.x+velocity.y*velocity.y)==0:
				velocity = dir*KNOCKBACK_AMOUNT/2
			else:
				velocity = dir*KNOCKBACK_AMOUNT/1.4
			pending_damage = null
